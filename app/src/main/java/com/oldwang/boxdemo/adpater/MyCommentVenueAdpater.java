...
public class MyCommentVenueAdpater extends BaseRecycleAdapter<MyCommentVenueBean> {
    ...
    LinearLayout venue_detail_layout;
    ImageView venue_image;
    TextView venue_content;
    ...
    public MyCommentVenueAdpater(Context context, List<MyCommentVenueBean> datas) {
        super(context, datas, R.layout.item_comment);
    }

    @Override
    protected void setData(RecycleViewHolder holder, MyCommentVenueBean s, final int position) {
        ...
        venue_image = holder.getItemView(R.id.venue_image);
        venue_content = holder.getItemView(R.id.venue_content);
        ...
        if(!TextUtils.isEmpty(s.getCommentContent())){
            tv_content.setText(s.getCommentContent());
        }
    ...
    public List<MyCommentVenueBean> GetChoiceData(){
        List<MyCommentVenueBean> choicelist = new ArrayList<>();
