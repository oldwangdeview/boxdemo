...
public class AuthenticationIDCardActivity extends BaseActivity {
    ...
    protected void initView() {
        setContentView(R.layout.activity_authentcationidcard);
    }

    @Override
    protected void initData() {
        super.initData();
        identifyingCode = getIntent().getStringExtra(Contans.INTENT_CODE);
        //为空为未提交审核
        if (TextUtils.isEmpty(identifyingCode) || identifyingCode.equals("-1")) {
            ...
            if (identifyingCode.equals("0")) {
                ...
                layout_view3.setVisibility(View.GONE);
            } else if (identifyingCode.equals("1")) {
                layout_view2.setVisibility(View.GONE);
    ...
    public void uploadPositiveImg(File head) {
        ...
        Observable observable = ApiUtils.getApi().uploadfile(AbsSuperApplication.getMemberId(), "member_info.positive_img", body)
    ...
    public void uploadNegativeImg(File head) {
        ...
        Observable observable = ApiUtils.getApi().uploadfile(AbsSuperApplication.getMemberId(), "member_info.negative_img", body)
                .compose(RxHelper.getObservaleTransformer())
                .subscribeOn(Schedulers.io())
                .doOnSubscribe(new Consumer<Disposable>() {
                    @Override
    ...
    private void upload() {
        ...
        String memberIdCard = user_basename.getText().toString();
        String realName = user_name.getText().toString();

        JsonObject jsonObject = new JsonObject();
        jsonObject.addProperty("memberId", AbsSuperApplication.getMemberId());
        jsonObject.addProperty("memberIdCard", memberIdCard);
        jsonObject.addProperty("realName", realName);

        RequestBean requestBean = HttpUtil.getRequsetBean(mContext, jsonObject);
        Observable observable =
                ApiUtils.getApi().memberUpdate(requestBean)
                        .compose(RxHelper.getObservaleTransformer())
                        .doOnSubscribe(new Consumer<Disposable>() {
                            @Override
                            public void accept(Disposable disposable) throws Exception {
        ...
        HttpUtil.getInstance().toSubscribe(observable, new ProgressSubscriber<Object>(mContext) {
            @Override
            protected void _onNext(StatusCode<Object> stringStatusCode) {
                ToastUtils.makeText("提交成功，请等待后台管理员审核");
    ...
    public void getIDcardZ() {
        Matisse.from(AuthenticationIDCardActivity.this)
    ...
    public static void startactivity(Context mContext, String identifyingCode) {
        Intent mIntent = new Intent(mContext, AuthenticationIDCardActivity.class);
        mIntent.putExtra(Contans.INTENT_CODE, identifyingCode);
    ...
    public void sumbit() {
        ...
        if (TextUtils.isEmpty(memberIdCard)){
            ToastUtils.makeText("请输入身份证号");
            return;
        }

        if (!UIUtils.personIdValidation(memberIdCard)){
            ToastUtils.makeText("身份证号不合法");
            return;
        }
