...
public class AppoinmentFragment extends BaseFragment {
    @BindView(R.id.recyclerview)
    YRecycleview recyclerview;

    AppoinmentAdpater madpater;
    ...
    public static AppoinmentFragment newInstance(int position) {
        AppoinmentFragment f = new AppoinmentFragment();
        Bundle b = new Bundle();
    ...
    /**
     * @param bookingStatus 预约状态(0:预约中,1:预约成功,-1预约失败,3完成)
     * 我的预约
     */
    private void getData(int bookingStatus) {

        JsonObject jsonObject = new JsonObject();
        jsonObject.addProperty("bookingStatus", bookingStatus);
        jsonObject.addProperty("memberId", AbsSuperApplication.getMemberId());
        jsonObject.addProperty("pageNum", page);
        jsonObject.addProperty("pageSize", size);
        RequestBean requestBean = HttpUtil.getRequsetBean(mContext, jsonObject);
        Observable observable =
                ApiUtils.getApi().myBespeakEvaluateList(requestBean)
    ...
    private void showData() {
        switch (state) {
            case STATE_NORMAL:
                madpater = new AppoinmentAdpater(mContext,datas,position);
    ...
    public void updateList(UpdateAppoinmentEvent event) {
        if (position == 1){
            refreshData();
        }
    }
